...
func main() {
	...
	primeFinderChannles := make([]<-chan int, CPUCount)

	for i := 0; i < CPUCount; i++ {
		primeFinderChannles[i] = primeFinder(done, randIntStream)
	}
	//fanIn
	fannedIntStream := fanIn(done, primeFinderChannles...)

	for rando := range take(done, fannedIntStream, 10) {
		fmt.Println(rando)
	}
